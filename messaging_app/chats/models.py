...
class Conversation(models.Model):
	"""
	Model representing a converation between users.
	Each converation can have multiple participants
	"""
...
class Message(models.Model):
	"""
	Moddel representing a message in a converation
	...
	sent_at = models.DateTimeField(auto_now_add=True)
	# Foreign key to link a message to a conversation and sender
	conversation = models.DateTimeField(auto_now_add=True)
	sender = models.ForeignKey(Conversation, on_delete=models.CASCADE, related_name='conversations')

	def __str__(self):
		return f"Message {self.id} from {self.sender.username} in converation {self.conversation.id}"
